...
class RentflowSupabaseService {
  ...
  UserRole _parseRole(String? r) => r == 'renter' ? UserRole.renter : UserRole.owner;
  ...
  Future<void> upsertProfileFromSignup({
    ...
    try {
      await _client.rpc('setup_my_profile', params: {
        'p_user_id': userId,
  ...
  Future<int> countTenantsCreatedSince(String ownerId, DateTime sinceUtc) async {
    try {
      final rows = await _client
          .from('tenants')
          .select('id')
  ...
  Future<List<PropertyItem>> fetchOwnerProperties(String ownerId) async {
    try {
      final props = await _client
          ...
          .eq('owner_id', ownerId)
          .order('name');
      final tenants = await _client.from('tenants').select('property_id').eq('owner_id', ownerId);
  ...
  Future<void> unlinkRenter(String tenantId) async {
    try {
      await _client
          .from('tenants')
          .update({'renter_profile_id': null}).eq('id', tenantId);
    } on Exception catch (e) {
  ...
  Future<void> markOwnerPaymentPaid(String paymentId) async {
    try {
      final row =
          await _client.from('payments').select('bill_id').eq('id', paymentId).maybeSingle();
  ...
  Future<List<ActivityItem>> fetchRecentActivities(String ownerId, {int limit = 6}) async {
    try {
      final rows = await _client
          .from('payments')
  ...
  Future<Map<String, dynamic>> fetchOwnerDashboardMetrics(String ownerId) async {
    try {
      final props = await _client.from('properties').select('id').eq('owner_id', ownerId);
      final tenants = await _client.from('tenants').select('id').eq('owner_id', ownerId);
      final payments = await _client.from('payments').select('amount, status, paid_at').eq('owner_id', ownerId);
  ...
  Future<String> createBillWithPayment({
    ...
    try {
      final billNumber = 'RF-${DateTime.now().millisecondsSinceEpoch % 100000000}';
  ...
  Future<RenterBill?> fetchBillForRenter({
    ...
    try {
      ...
      if ((tenant['renter_profile_id'] as String?) != renterProfileId) return null;
  ...
  RenterBill _mapBillRow(Map<String, dynamic> row) {
    ...
    return RenterBill(
      id: row['id'] as String,
      generatedOn: _fmtDate(created != null ? DateTime.tryParse(created) : null),
  ...
  Future<List<RenterBill>> fetchRenterBills(String renterProfileId) async {
    try {
      final tenantRows =
          await _client.from('tenants').select('id').eq('renter_profile_id', renterProfileId);
      final ids = (tenantRows as List<dynamic>)
          .map((e) => (e as Map<String, dynamic>)['id'] as String)
          .toList();
      if (ids.isEmpty) return [];
      final rows = await _client.from('bills').select().inFilter('tenant_id', ids).order(
            'created_at',
            ascending: false,
          );
      return (rows as List<dynamic>).map((r) => _mapBillRow(r as Map<String, dynamic>)).toList();
    } on Exception catch (e) {
      throw _wrap(e, 'Could not load bills');
    }
  }

  Future<List<RenterPaymentHistoryItem>> fetchRenterPaymentHistory(String renterProfileId) async {
    try {
      final tenantRows =
          await _client.from('tenants').select('id').eq('renter_profile_id', renterProfileId);
      final ids = (tenantRows as List<dynamic>)
      ...
      final rows = await _client
          .from('payments')
          .select('amount, status, paid_at, created_at, method, transaction_ref, bills ( bill_number )')
  ...
  Future<RenterLeaseSummary> fetchRenterLeaseSummary(String renterProfileId) async {
    try {
      final tenant = await _client
          .from('tenants')
          .select('id, owner_id, monthly_rent, property_id, properties ( name )')
      ...
      final landlord = await _client.from('profiles').select().eq('id', ownerId).maybeSingle();
      ...
      nextDueBill ??= billList.isNotEmpty ? billList.first as Map<String, dynamic> : null;
      ...
      for (final b in billList) {
        ...
        if (cd != null && cd.year == year) {
          ...
          if (bm['status'] == 'paid') annualPaid += (bm['amount'] as num?)?.toDouble() ?? 0;
        }
      }
      if (annualTotal == 0) annualTotal = monthly * 12;
      final annualRemaining = (annualTotal - annualPaid).clamp(0, double.infinity).toDouble();
      ...
      nextBillForSchedule ??= billList.isNotEmpty ? billList.first as Map<String, dynamic> : null;
      var nextPaymentDate = '';
      ...
      if (nextBillForSchedule != null) {
        ...
        nextPaymentAmount = (nextBillForSchedule['amount'] as num?)?.toDouble() ?? monthly;
      }
  ...
  Future<PayRentInvoiceSummary?> fetchPayRentSummary({
    ...
    final bill = await fetchBillForRenter(renterProfileId: renterProfileId, billId: billId);
    if (bill == null) return null;
    try {
      final billRow = await _client.from('bills').select('bill_number').eq('id', billId).maybeSingle();
  ...
  /// Marks the payment and linked bill paid; returns the new transaction reference.
